...
- `scripts/` holds the Python automation entry points (auth, notebook library, query flow). Use `scripts/run.py` as the wrapper.
- `references/` contains deeper usage and troubleshooting docs.
- `docs/plans/` stores design proposals; review before changing core automation architecture.
- `images/` is used for README/documentation assets.
- `SKILL.md` and `CLAUDE.md` define how the skill runs and how to develop locally.
- Runtime data is written to `data/` under the skill directory and is git-ignored because it contains auth/session state.

## Build, Test, and Development Commands

There is no build step; scripts self-manage a local `.venv` through `run.py`.
...
# Notebook library operations
python scripts/run.py notebook_manager.py list

# Query NotebookLM
python scripts/run.py ask_question.py --question "What does this notebook cover?"
```

If the wrapper fails, fall back to manual setup:
```bash
python -m venv .venv
...
- Python, 4-space indentation, PEP 8 style where practical.
- Use `snake_case` for modules and functions; files follow `*_manager.py`, `*_session.py`.
- Keep CLI output concise and action-oriented; prefer short docstrings for key functions.
- Environment configuration lives in `.env` with uppercase keys (see `CLAUDE.md`).

## Testing Guidelines

No automated test suite; validate changes by running the relevant scripts. For example:
- Auth changes: `python scripts/run.py auth_manager.py setup`
- Query flow: `python scripts/run.py ask_question.py --question "..." --show-browser`
- Cleanup behavior: `python scripts/run.py cleanup_manager.py --confirm`

## Commit & Pull Request Guidelines

- Commit messages commonly use conventional prefixes: `feat:`, `fix:`, `docs:`, `chore:` plus a short summary.
- PRs should include a clear description, manual test commands run, and links to related issues.
- If user-facing behavior or docs change, update `README.md` and consider a `CHANGELOG.md` entry.

## Design Notes

The most recent architecture exploration is in `docs/plans/2026-01-18-agent-browser-integration-design.md`. Use it as the starting point for discussions about replacing Patchright or changing the automation stack.
<!-- PRPM_MANIFEST_START -->
